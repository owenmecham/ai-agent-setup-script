...
async function main() {
  ...
  switch (command) {
    ...
    case 'google-auth': {
      ...
      console.log('  3. Create an OAuth 2.0 Client ID (type: Desktop app)');
      ...
      console.log('  5. Under OAuth consent screen → Audience, add your Google account as a test user');
      console.log('  6. Under OAuth consent screen → Data Access, add scopes for all enabled APIs');
      console.log('');
      ...
      if (!hasCredFiles()) {
        ...
        // user_google_email is required by the schema; value triggers OAuth redirect before use
        sendJsonRpc({
